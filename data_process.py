...
class SentenceDataset(Dataset):
    def __init__(self, opt, data_path, text_tokenizer, photo_path, image_transforms, data_type, data_translation_path=None, image_coordinate=None):
        ...
        if opt.text_model == 'bert-base':
            ...
            # self.text_token_list = [text_tokenizer.tokenize('[CLS]' + text + '[SEP]') for text in
            #                         tqdm(self.text_list, desc='convert text to token')]
            self.text_token_list = [{text if len(text) < opt.word_length else text[0: opt.word_length]: text_tokenizer.tokenize('[CLS]' + text + '[SEP]')} for text in
                                    tqdm(self.text_list, desc='convert text to token')]
        ...
        for text in self.text_token_list:
            ...
            for key, val in text.items():
                val = val if len(val) < opt.word_length else val[0: opt.word_length]
                val = text_tokenizer.convert_tokens_to_ids(val)
                self.text_to_id.append({key: val})


        self.text_translation_id_to_token_list = {
        ...
        self.text_translation_to_id = {index: text_tokenizer.convert_tokens_to_ids(text_token) for index, text_token in
                                       self.text_translation_id_to_token_list.items()}
    ...
    def __getitem__(self, index):
        image_path = (self.photo_path + '/' + str(self.data_id_list[index]) + '.jpg').replace('//','/')
        ...
        # text, text_to_id = self.text_to_id[index].items()
        # print(text, text_to_id)
        ...
        return self.text_to_id[index], image_origin, self.label_list[index], self.text_translation_to_id[self.data_id_list[index]], image_augment, image_path,
    ...
    def _collate(self, batch_data):
        ...
        for b in batch_data:
            for key, val in b[0].items():
                text_to_id.append(torch.LongTensor(val))
                text.append(key)

        # text_to_id = [torch.LongTensor(b[0]) ]
        ...
        image_path = [np.array(b[5]) for b in batch_data]
        ...
        return [text_to_id, text], torch.LongTensor(bert_attention_mask), image_origin, torch.LongTensor(text_image_mask), label, \
               text_translation_to_id, torch.LongTensor(tran_bert_attention_mask), image_augment, torch.LongTensor(tran_text_image_mask), target_labels, image_path
